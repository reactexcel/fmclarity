...
export default UserViewRelationEdit = React.createClass({
  ...
  render() {
    ...
    if (userRole === 'caretaker') {
      ...
      if (group.type === 'real estate') {
        roles.push("property manager");
      }
    ...
    if (relation) {
      ...
      return (
        <div>
          ...
          {(_.contains(['portfolio manager', 'fmc support'], userRole) ) &&
          _.contains(['manager', 'caretaker'], role) && group && group.type != 'contractor' ?
            <Text
              value={ threshold }
