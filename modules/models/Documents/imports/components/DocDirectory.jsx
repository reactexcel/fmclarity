...
export default function DocDirectory( props ) {
    ...
    return (
        <div>
        {visibility==true?<div id={'doc-icon-'+idx} key={idx} className="doc-icon"  style={{paddingLeft:'12px',backgroundColor:(folder.folderColor?folder.folderColor:(docAlmostExpires?'#ffffcc':(docExpired?'#ffe0e0':'#ffffff')))}} onMouseOver={()=>{
            ...
            <span style={{display:"inline-block",minWidth:"18px",color:'gray',paddingRight:"24px"}}><i style={{backgroundColor:(folder.folderColor?folder.folderColor:(docAlmostExpires?'#ffffcc':(docExpired?'#ffe0e0':'#ffffff'))),borderRadius:'60px',padding:'7px',color:color,fontSize:'20px',marginTop:'10px'}} id={'icon-'+idx} className={icon}></i></span>
