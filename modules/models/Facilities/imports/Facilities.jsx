...
Facilities.actions({
  ...
  updateBookingForArea: {
    ...
    method: function (newRequest) {
      ...
      let level = newRequest.level;
      let area = newRequest.area;
      let identifier = newRequest.identifier;
      let booking = newRequest.bookingPeriod;
        ...
        if (service != null && serviceIndex != null) {
          ...
          rule.map((r, idx) => {
            ...
            if (!r.subservice) {
              ...
              for (let i in services[serviceIndex].children) {
          ...
          rule.map((r, idx) => {
            ...
            if (!r.subservice) {
              ...
              if (services[len].children) {
                for (let i in services[len].children) {
  ...
  updateComplianceRule: {
    ...
    helper: (facility, rulePosition, updatedRule, serviceName, servicePosition, subservicePosition) => {
      ...
      if (updatedRule.document) {
        ...
        let query = JSON.stringify(updatedRule.document.query);
          ...
          if (supplier && supplier._id) {
            ...
            if (supplier) {
              ...
              if (selectedTeamSuppliers.length) {
                if (_.find(selectedTeamSuppliers, function (obj) {
                    return obj._id === supplier._id;
                  })) {
                  flag = false;
                }
              }
...
function invitePropertyManager(team, email, ext) {
  ...
  if (user) {
    ...
    let name = DocMessages.isValidEmail(email);
    if (name) {
      if (Meteor.isServer) {
