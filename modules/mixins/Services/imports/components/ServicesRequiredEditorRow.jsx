...
const ServicesRequiredEditorRow = React.createClass({
  ...
  getMeteorData() {
    let service, supplier, suppliers, defaultContact;
    service = this.props.service;
    suppliers = this.props.suppliers;
    if (service.data && service.data.supplier) {
      let q = service.data.supplier;
      if (q._id) {
  ...
  updateServiceName(event) {
    //Modal.hide();
    let service = this.data.service;
    let newValue = event ? event.target.value : this.data.service.name;
  ...
  showSupplierModal(supplier) {
    ...
    Modal.replace({
      content: <TeamStepper item={ supplier } facility={ facility }
    ...
    return (
      <div>
        <div className="services-editor-col services-editor-col-service"
             style={{width: '70%'}}>
          {clickExpand ?
            <span onClick={clickExpand} className="services-editor-expand-icon"><i
              className="fa fmc-fa-icon-expand"></i></span> : null}
          ...
          <input
            ...
            onBlur={(evt) => {
              if ($('#' + this.props.id).val().trim() == "") {
                let confirmExit = confirm("Exiting editor. Service will be removed.");
                if (confirmExit == true) {
                  this.removeService();
          ...
          />
          {! readOnly ?
            <span className="services-editor-delete-icon"
                  onClick={
                    () => {
                    ...
                    } }><i title="Configure" className="fa fa-cogs" aria-hidden="true"/></span>
            : null}
        </div>
        <div style={{width: "30%", float: "left"}}>
          <div className="services-editor-col services-editor-col-supplier"
               style={{width: "100%", cursor: "default"}}>
            {supplier ? <ContactCard item={supplier}/> : null}
          </div>
        </div>
      </div>
    )
